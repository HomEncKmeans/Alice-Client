...
KClientT1V2::KClientT1V2(unsigned p, unsigned g, unsigned logQ, const string &data, const string &u_serverIP,
...
void KClientT1V2::connectToTServer() {
    ...
    if (connect(this->t_serverSocket, (struct sockaddr *) &t_server_address, sizeof(t_server_address)) < 0) {
        ...
        print("KClientT1V2 CONNECTED TO TSERVER");

    }

}

void KClientT1V2::connectToUServer() {
    ...
    if (connect(this->u_serverSocket, (struct sockaddr *) &u_server_address, sizeof(u_server_address)) < 0) {
        ...
        print("KClientT1V2 CONNECTED TO USERVER");

    }

}

bool KClientT1V2::sendMessage(string message, int socket) {
...
bool KClientT1V2::sendStream(ifstream data, int socket) {
...
string KClientT1V2::receiveMessage(const int &socket, int buffersize) {
...
void KClientT1V2::log(int socket, string message) {
    if(this->verbose){
    sockaddr address;
...
void KClientT1V2::sendEncryptionParamToUServer() {
...
void KClientT1V2::sendEncryptedDataToUServer() {
    this->connectToUServer();
...
void KClientT1V2::receiveResult() {
...
void KClientT1V2::calculateCentroid(int socketFD) {
...
Plaintext KClientT1V2::newCentroidCoef(const Plaintext &sum, long mean) {
...
ifstream KClientT1V2::centroidCoefToStream(const Ciphertext &centroid) {
...
ifstream KClientT1V2::receiveStream(int socketFD, string filename) {
    uint32_t size;
    auto *data = (char *) &size;
    if (recv(socketFD, data, sizeof(uint32_t), 0) < 0) {
